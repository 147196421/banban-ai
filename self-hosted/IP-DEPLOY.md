...
本方式只在服务器新增一个办办AI网站进程，保留既有域名站点、Nginx、检测服务和证书。网站端口 3080；检测服务保持现有的本机端口 8787。不要运行系统更新、重启服务器或安装 Chromium 系统依赖。
...
3. 给新网站单独设置可写的任务目录和环境变量：`BANBAN_WEB_DATA_DIR` 指向可写目录，`BANBAN_SELF_HOSTED=1`，`BANBAN_TEST_SERVICE_TOKEN` 与现有检测服务 `SERVICE_TOKEN` 相同。密钥只在服务器环境文件中使用，不要写到仓库或输出日志。若现有 8787 服务尚未配置，先报告现状；网站页面可以先部署，但测试需要它。
